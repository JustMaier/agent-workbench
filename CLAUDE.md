...
- **Deployment**: Self-hosted via Node.js (optionally with PM2)
...
public/
  ...
  api.js                # fetchConfig(), generate() with SSE streaming
...
- **SSE streaming**: Server parses OpenRouter's SSE stream and re-emits as `data: {"type":"text_delta","delta":"..."}` events.
...
- Model list is hardcoded in `server.mjs` `MODELS` array. Update there to change available models.
